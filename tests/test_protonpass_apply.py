"""Tests for ``hermes_protonpass.source.apply`` + the CLI / env_loader
...
def test_apply_missing_token(monkeypatch):
    ...
    result = pp.apply_protonpass_secrets(enabled=True, vault="V", auto_install=False)
            ...
            "items": [
                {
                    "content": {
                        ...
                        "extra_fields": [
                            {"name": "New", "content": {"Text": "new-value"}}
                        ],
                    }
                }
            ]
...
def _pp_cli():
    from hermes_protonpass import cli
...
def test_cli_setup_scalar_protonpass_config_does_not_crash(hermes_home, monkeypatch):
    ...
    vault_section = saved["cfg"]["secrets"]["protonpass_vault"]
    assert isinstance(pp_section, dict)
    assert isinstance(vault_section, dict)
    assert pp_section["enabled"] is False
    assert vault_section["enabled"] is True
    assert vault_section["vault"] == "V"


def test_cli_setup_migrates_legacy_combined_vault_config(hermes_home, monkeypatch):
    cli = _pp_cli()
    monkeypatch.setenv("LEGACY_PROTON_TOKEN", "svc")
    cfg = {
        "secrets": {
            "protonpass": {
                "enabled": True,
                "vault": "Legacy Vault",
                "service_token_env": "LEGACY_PROTON_TOKEN",
                "auto_install": False,
            }
        }
    }
    ...
    monkeypatch.setattr(cli, "save_env_value", lambda *_args: None)
    monkeypatch.setattr(cli, "get_env_path", lambda: "/tmp/.env")
    binary = hermes_home / "bin" / "pass-cli"
    binary.parent.mkdir(parents=True, exist_ok=True)
    binary.write_text("", encoding="utf-8")
    monkeypatch.setattr(cli.pp, "find_pass_cli", lambda **_kwargs: binary)
    monkeypatch.setattr(cli, "_pp_version", lambda _binary: "pass-cli 2.1.1")
    monkeypatch.setattr(
        cli.pp, "fetch_protonpass_secrets", lambda **_kwargs: ({"K": "v"}, [])
    )

    rc = cli.cmd_pp_setup(
        argparse.Namespace(vault=None, token_env="LEGACY_PROTON_TOKEN")
    )

    assert rc == 0
    mapped = saved["cfg"]["secrets"]["protonpass"]
    bulk = saved["cfg"]["secrets"]["protonpass_vault"]
    assert "vault" not in mapped
    assert mapped["enabled"] is False
    assert bulk["enabled"] is True
    assert bulk["vault"] == "Legacy Vault"
    assert bulk["service_token_env"] == "LEGACY_PROTON_TOKEN"
...
def test_cli_setup_defers_token_save_until_fetch_succeeds(hermes_home, monkeypatch):
    ...
    monkeypatch.setattr(
        cli,
        "save_config",
        lambda c: saved_calls.__setitem__("config", saved_calls["config"] + 1),
    )
    monkeypatch.setattr(
        cli,
        "save_env_value",
        lambda k, v: saved_calls.__setitem__("env", saved_calls["env"] + 1),
    )
    monkeypatch.setattr(cli, "get_env_path", lambda: "/tmp/.env")
...
def test_cli_setup_saves_token_after_successful_fetch(hermes_home, monkeypatch):
    ...
    monkeypatch.setattr(cli, "_pp_version", lambda b: "pass-cli 2.1.1")
    monkeypatch.setattr(
        cli.pp, "fetch_protonpass_secrets", lambda **kw: ({"K": "v"}, [])
    )
...
def test_cli_setup_non_tty_vault_eof_leaves_disabled(hermes_home, monkeypatch):
    ...
    assert saved["cfg"]["secrets"]["protonpass_vault"]["enabled"] is False
    assert saved["cfg"]["secrets"]["protonpass_vault"]["vault"] == ""
    assert env_saves == [("PROTON_PASS_PERSONAL_ACCESS_TOKEN", "svc")]
...
def test_cli_status_scalar_secrets_does_not_crash(hermes_home, monkeypatch):
    """A top-level scalar ``secrets: true`` must NOT
...
def test_pp_version_uses_scrubbed_env_probe(monkeypatch):
    ...
    def fake_run(cmd, *, extra_env, timeout, **kwargs):
    ...
    monkeypatch.setattr(cli._install, "run_secret_cli", fake_run)
...
def test_protonpass_source_coerces_enabled_string_false():
    from hermes_protonpass.source import ProtonPassSource
...
def test_env_loader_string_false_disables_protonpass(tmp_path, monkeypatch):
    ...
    import hermes_protonpass.source as pp_module
