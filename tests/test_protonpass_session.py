"""Tests for ``hermes_protonpass.source.session``.
...
def test_session_login_failure_detail_uses_stderr_only(
...
def test_run_pass_cli_uses_shared_hermes_helper(monkeypatch):
    """Provider subprocesses delegate to Hermes' audited public helper."""
    captured = {}
    ...
    monkeypatch.setattr(pp_session, "run_secret_cli", fake_run)
    result = pp_session._run_pass_cli(["pass-cli", "info"], {"E": "1"})
    assert result.returncode == 0
    assert captured == {
        "cmd": ["pass-cli", "info"],
        "extra_env": {"E": "1"},
        "timeout": pp_session._PASS_CLI_RUN_TIMEOUT,
    }
...
def test_child_env_empty_override_falls_back_to_secure_default(
...
def test_session_dir_suffix_unconditional(hermes_home):
    ...
    assert bare.name == (f"protonpass-session-{pp_session._token_fingerprint('')}")
