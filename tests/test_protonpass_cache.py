"""Tests for ``hermes_protonpass.source.cache``.
...
def test_cache_home_accepts_string_path(hermes_home):
    assert pp_cache._disk_cache_path(str(hermes_home)) == (
        hermes_home / "cache" / "protonpass_cache.json"
    )


def test_corrupt_utf8_disk_cache_is_a_miss(hermes_home):
    cache_key = pp_cache.build_cache_key("svc", "V", {}, hermes_home)
    cache_path = pp_cache._disk_cache_path(hermes_home)
    cache_path.parent.mkdir(parents=True, exist_ok=True)
    cache_path.write_bytes(b"\xff")

    assert pp_cache._read_disk_cache(cache_key, 300, hermes_home) is None
