    ...
    * An already-set var is kept unless ``override_existing`` is True.  (Skip
    ...
    for name in sorted(secrets):
        ...
        if name == token_env:
            plan.append(
                PlanItem(
                    name=name, value=value, applied=False, reason=SKIP_BOOTSTRAP_TOKEN
                )
            )
            continue
        ...
        if already and not override_existing:
            plan.append(
                PlanItem(name=name, value=value, applied=False, reason=SKIP_ALREADY_SET)
            )
            continue
        plan.append(PlanItem(name=name, value=value, applied=True, overrides=already))
    return plan
    ...
    *,
    ...
    if not service_token:
        result.error = (
            ...
            "not set.  Run `hermes protonpass setup`."
        )
    ...
    if not ProtonPassConfig(vault=vault, env_refs=env_refs).has_fetch_target():
        result.error = (
            ...
            "(MODE B).  Run `hermes protonpass setup`."
        )
        result.error_kind = ErrorKind.NOT_CONFIGURED
        return result
