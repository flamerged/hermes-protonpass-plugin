"""CLI handlers for ``hermes protonpass ...``.
...
Subcommands:
    ...
    disable  — disable both mapped and bulk Proton Pass sources
...
from . import source as pp
from .source import install as _install
from .source import (
    SKIP_ALREADY_SET,
...
from .source.config import is_valid_env_name, strip_bootstrap_ref
from .source.session import _redact_token
...
# ---------------------------------------------------------------------------
# config-section access — normalization for both registered source sections
# ---------------------------------------------------------------------------
#
...
def _read_protonpass_vault_section(cfg: dict) -> object:
    """Read ``secrets.protonpass_vault``, with CLI-only legacy migration."""
    secrets = cfg.get("secrets")
    if not isinstance(secrets, dict):
        return None
    configured = secrets.get("protonpass_vault")
    if configured is not None:
        return configured
    mapped = secrets.get("protonpass")
    if isinstance(mapped, dict) and mapped.get("vault"):
        return mapped
    return None
...
def _ensure_protonpass_vault_section(cfg: dict) -> dict:
    """Return a mutable ``secrets.protonpass_vault`` dict."""
    if not isinstance(cfg.get("secrets"), dict):
        cfg["secrets"] = {}
    secrets = cfg["secrets"]
    if not isinstance(secrets.get("protonpass_vault"), dict):
        secrets["protonpass_vault"] = {}
    return secrets["protonpass_vault"]


# ---------------------------------------------------------------------------
# argparse wiring — registered through PluginContext.register_cli_command
# ---------------------------------------------------------------------------


def register_protonpass_cli(parent_parser: argparse.ArgumentParser) -> None:
    """Attach Proton Pass actions to the plugin's top-level parser.

    Hermes exposes this parser as ``hermes protonpass`` through the public
    plugin CLI registration surface.
    """
    sub = parent_parser.add_subparsers(dest="protonpass_command")
...
def cmd_pp_setup(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        console.print("  Manual install: https://proton.me/download/pass-cli")
        return 1
    ...
    cfg = load_config()
    mapped_section = _read_protonpass_section(cfg)
    vault_section = _read_protonpass_vault_section(cfg)
    parsed_cfg = ProtonPassConfig.from_mapping(mapped_section)
    parsed_vault_cfg = ProtonPassConfig.from_mapping(vault_section)
    # A hand-edited config.yaml may carry a non-dict at `secrets` or
    ...
    # Read before normalizing so setup can migrate the old combined
    # ``secrets.protonpass.vault`` layout into the bulk source section. Pick the
    # active source's token variable, unless --token-env explicitly selects a
    # new one, and align both source sections when saving.
    secrets_cfg = _ensure_protonpass_section(cfg)
    vault_secrets_cfg = _ensure_protonpass_vault_section(cfg)
    configured_token_env = (
        parsed_cfg.service_token_env
        if parsed_cfg.enabled or parsed_cfg.env_refs
        else parsed_vault_cfg.service_token_env
    )
    token_env = (args.token_env or configured_token_env).strip()
    ...
    console.print(
        f"  [dim]The token will be saved to {get_env_path()} (never config.yaml) "
    ...
    vault = (args.vault or parsed_vault_cfg.vault or "").strip()
    if not vault:
        try:
            vault = console.input(
    ...
    if not has_target:
        console.print(
            ...
            "config.yaml, then\n"
            "  re-run [cyan]hermes protonpass setup[/cyan]. Leaving the "
    ...
    if not has_target:
        ...
        if not secrets:
            console.print("  [yellow]Fetch succeeded but returned no secrets.[/yellow]")
        else:
    ...
    console.print(
        f"  [green]OK[/green] token stored in {get_env_path()} as {token_env}"
    )
    ...
    secrets_cfg["enabled"] = bool(env_refs)
    secrets_cfg.pop("vault", None)
    secrets_cfg["service_token_env"] = token_env
    ...
    vault_secrets_cfg["enabled"] = bool(vault)
    vault_secrets_cfg["vault"] = vault
    vault_secrets_cfg["service_token_env"] = token_env
    vault_secrets_cfg.setdefault("cache_ttl_seconds", 300)
    vault_secrets_cfg.setdefault("override_existing", False)
    vault_secrets_cfg.setdefault("auto_install", True)
    save_config(cfg)

    console.print()
    if has_target:
        console.print(
            "[green]Proton Pass is enabled.[/green]  Mapped refs and bulk vault "
            "export are registered separately so Hermes applies the correct "
            "precedence to each."
        )
    else:
        console.print(
    ...
    console.print(
        "  Status:  [cyan]hermes protonpass status[/cyan]\n"
        "  Refresh: [cyan]hermes protonpass sync[/cyan]\n"
        "  Disable: [cyan]hermes protonpass disable[/cyan]"
    )
    return 0
...
def cmd_pp_status(args: argparse.Namespace) -> int:
    ...
    vault_cfg = ProtonPassConfig.from_mapping(_read_protonpass_vault_section(cfg))

    token_env = (
        pp_cfg.service_token_env if pp_cfg.enabled else vault_cfg.service_token_env
    )
    ...
    table.add_column("")
    table.add_row("Mapped source enabled", _yn(pp_cfg.enabled))
    table.add_row("Bulk source enabled", _yn(vault_cfg.enabled))
    table.add_row("Token env var", token_env)
    table.add_row("Token in env", _yn(token_set))
    table.add_row("Vault (MODE A)", vault_cfg.vault or "[dim](unset)[/dim]")
    table.add_row(
        "Env refs (MODE B)",
    ...
    if not pp_cfg.enabled and not vault_cfg.enabled:
        console.print("\n  Run [cyan]hermes protonpass setup[/cyan] to enable.")
        return 0
    if not token_set:
        console.print(
    ...
    if not pp_cfg.env_refs and not vault_cfg.vault:
        console.print(
...
def cmd_pp_sync(args: argparse.Namespace) -> int:
    ...
    pp_cfg = ProtonPassConfig.from_mapping(_read_protonpass_section(cfg))
    vault_cfg = ProtonPassConfig.from_mapping(_read_protonpass_vault_section(cfg))
    mapped_active = pp_cfg.enabled and bool(pp_cfg.env_refs)
    vault_active = vault_cfg.enabled and bool(vault_cfg.vault)
    if not mapped_active and not vault_active:
        console.print(
            "[yellow]Proton Pass integration is disabled.  Run "
            "`hermes protonpass setup` first.[/yellow]"
        )
        return 1
    ...
    if (
        mapped_active
        and vault_active
        and pp_cfg.service_token_env != vault_cfg.service_token_env
    ):
        console.print(
            "[red]Mapped and bulk Proton Pass sources use different token env "
            "vars. Run `hermes protonpass setup` to align them.[/red]"
        )
        return 1
    token_env = (
        pp_cfg.service_token_env if mapped_active else vault_cfg.service_token_env
    )
    ...
    if not vault_active and not env_refs:
        console.print(
    ...
    try:
        secrets, warnings = pp.fetch_protonpass_secrets(
            service_token=token,
            vault=vault_cfg.vault if vault_active else "",
            env_refs=env_refs,
            use_cache=False,
            auto_install=all(
                source_cfg.auto_install
                for active, source_cfg in (
                    (mapped_active, pp_cfg),
                    (vault_active, vault_cfg),
                )
                if active
            ),
            home_path=get_hermes_home(),
    ...
    override = pp_cfg.override_existing or vault_cfg.override_existing or args.apply
    plan = plan_application(
    ...
    for item in plan:
        if not item.applied:
            ...
            continue
        suffix = (
            (" (overrode)" if item.overrides else "")
            if args.apply
            else (" (overrides)" if item.overrides else "")
        )
        if args.apply:
...
def cmd_pp_disable(args: argparse.Namespace) -> int:
    ...
    vault_cfg = _ensure_protonpass_vault_section(cfg)
    pp_cfg["enabled"] = False
    vault_cfg["enabled"] = False
    save_config(cfg)
    console.print(
